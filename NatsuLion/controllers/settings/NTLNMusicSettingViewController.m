...
@interface NTLNMusicSettingViewController(Private)
...
@implementation NTLNMusicSettingViewController

- (void)setupPrototypes {
	if (groups != nil) return;
	
	UICPrototypeTableCellTextInput *c1 = [UICPrototypeTableCell cellForTextInput:@"Music" 
																 withPlaceholder:@"optional" 
															 withUserDefaultsKey:NTLN_PREFERENCE_MUSIC];
...
- (id)initWithStyle:(UITableViewStyle)style {
	if (self = [super initWithStyle:style]) {
		[self.navigationItem setTitle:@"Music"];
	}
	return self;
}
